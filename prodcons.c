...
// Synchronization
static pthread_cond_t cv_load[NROF_PRODUCERS]; // Renamed to array for targeted signaling
...
// Mapping to avoid "uninterested" wakeups
...
producer(void *arg)
{
	...
	{
		// Map this item to me so others know who to wake up
		pthread_mutex_lock(&mutex);
		item_to_producer[item] = my_id;
		...
		// Strictly wait until it is MY turn and there is SPACE
		while (buffer_count >= BUFFER_SIZE || item != expected_item)
		{
			pthread_cond_wait(&cv_load[my_id], &mutex);
		}
